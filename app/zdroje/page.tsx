...
export default function SourcesPage() {
  return <main id="hlavni-obsah">
    <section className="page-hero shell"><div className="page-hero-grid"><div><p className="eyebrow"><span /> Zdroje a opravy</p><h1>Nevěř nám naslepo. Otevři si zdroj.</h1></div><p className="lead">Pravidla se mění. Proto oddělujeme oficiální pravidlo od praktického odhadu a u citlivých informací uvádíme datum kontroly.</p></div></section>
    ...
    <section className="content-section shell"><div className="content-grid"><div><p className="eyebrow"><span /> Jak pracujeme</p><h2>Tři druhy informací.</h2></div><div className="source-methods"><article><span>01</span><h3>Oficiální pravidlo</h3><p>Částka, termín nebo právní podmínka. Odkazujeme přímo na úřad, školu nebo veřejnou službu.</p></article><article><span>02</span><h3>Praktický odhad</h3><p>Například doporučený startovní rozpočet. Vysvětlujeme předpoklady a netváříme se, že jde o zákonné minimum.</p></article><article><span>03</span><h3>Zkušenost</h3><p>Co nám fungovalo nebo co bývá těžké. Pomáhá s orientací, ale nepřebíjí individuální požadavek instituce.</p></article></div></div></section>
    <section className="sources-directory"><div className="shell"><div className="tool-heading"><div><p className="eyebrow eyebrow-light"><span /> Přímé odkazy</p><h2>Ověřuj tam, kde se rozhoduje.</h2></div><p>U programu vždy otevři i web konkrétní univerzity. Katalog je rozcestník, nikoli závazné rozhodnutí o přijetí.</p></div><div className="source-group-grid">{sourceGroups.map((group) => <article key={group.title}><h3>{group.title}</h3>{group.links.map(([label, href]) => <a key={href} href={href} target="_blank" rel="noreferrer">{label}<span>↗</span></a>)}</article>)}</div></div></section>
    <section className="content-section shell"><div className="content-grid"><div><p className="eyebrow"><span /> Našel/a jsi chybu?</p><h2>Oprava je součást projektu.</h2></div><div className="content-body"><p>Pokud je odkaz nefunkční nebo se pravidlo změnilo, pošli nám stránku zdroje, datum kontroly a krátký popis. Do zveřejnění kontaktní adresy prosím ověřuj kritické informace přímo se školou nebo úřadem.</p><div className="callout callout-yellow"><strong>Co pravidelně kontrolovat</strong><p>Termíny přihlášek, otevřené intake, SU sazby a worker pravidla, housing garance, depozity u jazykových kurzů a odkazy univerzit.</p></div></div></div></section>
  </main>;
}
