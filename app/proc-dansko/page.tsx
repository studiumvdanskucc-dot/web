...
export default function WhyDenmarkPage() {
  return <main id="hlavni-obsah">
    <section className="page-hero why-hero shell"><div className="page-hero-grid"><div><p className="eyebrow"><span /> Proč Dánsko</p><h1>Více samostatnosti.<br />Méně memorování.<br /><em>Více praxe.</em></h1></div><p className="lead">Dánsko nabízí kvalitní programy v angličtině, praktickou výuku a mezinárodní prostředí. Studium má mnoho výhod, ale je důležité počítat také s vysokými životními náklady a hledáním bydlení.</p></div></section>
    ...
    <section className="content-section shell"><div className="content-grid"><div><p className="eyebrow"><span /> Jak probíhá výuka</p><h2>Samostatná práce, diskuse a týmové projekty.</h2></div><div className="content-body"><p>Dánská výuka často stojí na samostatnosti, diskusi a řešení skutečných problémů. Vyučující ti poradí, ale velkou část odpovědnosti za přípravu a průběh projektu máš ty. Počítej se skupinovými projekty, prezentacemi, četbou a pravidelnou zpětnou vazbou.</p><div className="cards-grid"><div className="info-card"><h3>Méně formální prostředí</h3><p>Studenti často oslovují vyučující křestním jménem. Na přípravě, respektu a spolehlivosti to nic nemění.</p></div><div className="info-card"><h3>Skupinové projekty</h3><p>Naučíš se rozdělit práci, dávat zpětnou vazbu a vysvětlit, proč jste zvolili konkrétní řešení.</p></div></div><div className="callout callout-mint"><strong>Tento styl ti může vyhovovat, pokud</strong><p>se chceš aktivně zapojovat, umíš se zeptat a raději hledáš řešení, než se učíš text nazpaměť.</p></div></div></div></section>
    ...
    <section className="honest-grid shell content-section"><div><p className="eyebrow"><span /> Výhody</p><h2>Praktické studium a větší samostatnost.</h2><ul><li>programy v angličtině</li><li>otevřenější komunikace s vyučujícími</li><li>studentské spolky, sport a akce na kampusu</li><li>města vhodná pro kolo a blízkost přírody</li></ul></div><div><p className="eyebrow eyebrow-light"><span /> Nevýhody</p><h2>Vysoké počáteční náklady a hledání práce.</h2><ul><li>vysoký depozit a nedostatek bydlení</li><li>nárok na SU není automatický</li><li>dlouhá, tmavá a větrná zima</li><li>bez dánštiny může být hledání práce obtížnější</li></ul></div></section>
